...
def run_ret_tool(out_dir, run_ret=True, **kwargs):
    dir_path = os.path.dirname(os.path.realpath(__file__))
    print dir_path
    ret_tool_dic = {'time_start': '20170323', 'time_end': '20170720', 'site_nml': dir_path+'/site_new_test.nml', 'states_file':
    ...
    if run_ret is True:
        subprocess.call(['cp', '-R', dir_path+'/ret_code_lai', out_dir])
        os.chdir(out_dir)
        subprocess.call(['make', 'setup'])
        cmd = ['bin/rs_pre.py', 'pre_general']
        for key in ret_tool_dic.keys():
...
def print_ret_tool(out_dir, run_ret=True, **kwargs):
    dir_path = os.path.dirname(os.path.realpath(__file__))
    print dir_path
    ret_tool_dic = {'time_start': '20170323', 'time_end': '20170720', 'site_nml': dir_path+'/site_new_test.nml', 'states_file':
                    False, 'obs_s1': False, 'obs_s2': False, 'no_use_prior': False, 'no_use_states': False, 'gtol':
...
def print_cmd(exp_no, exp_tup, point='508_med', run_ret=True, field_obs=True):
    ...
    try:
        ...
        s2_str = 'nos2'
    save_dir = '/export/cloud/nceo/users/if910917/s3_exps/paper_short2/'+point+'/'+prior+'_'+s1_str+'_'+s2_str
    ret_dic = print_ret_tool(save_dir, run_ret=run_ret, time_start=exp_tup[0], time_end=exp_tup[1], states_file=exp_tup[2],
